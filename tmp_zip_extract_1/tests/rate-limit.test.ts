...
describe("rate-limit", () => {
  ...
  describe("checkRateLimit", () => {
    ...
    it("allows requests under limit", () => {
      ...
      expect(checkRateLimit(req, maxPerWindow, windowMs)).toBe(true);
      expect(checkRateLimit(req, maxPerWindow, windowMs)).toBe(true);
      expect(checkRateLimit(req, maxPerWindow, windowMs)).toBe(true);
    });
    ...
    it("rejects when over limit", () => {
      ...
      checkRateLimit(req, maxPerWindow, windowMs);
      checkRateLimit(req, maxPerWindow, windowMs);
      checkRateLimit(req, maxPerWindow, windowMs);
      expect(checkRateLimit(req, maxPerWindow, windowMs)).toBe(false);
    });
    ...
    it("different IPs have separate counters", () => {
      ...
      checkRateLimit(reqA, 1, windowMs);
      checkRateLimit(reqA, 1, windowMs); // over for A
      expect(checkRateLimit(reqB, 1, windowMs)).toBe(true);
    });
  });
});
