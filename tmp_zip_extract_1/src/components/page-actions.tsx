...
export default function PageActions({ pageId, initialUpvotes = 0, className }: Props) {
  ...
  const onUpvote = async () => {
    ...
    setUpvoting(true);
    try {
      const res = await fetch(`/api/pages/${pageId}/upvote`, { method: "POST" });
    ...
    <div className={className}>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          onClick={onUpvote}
          disabled={upvoting}
          className="inline-flex items-center gap-1 rounded-full border border-neutral-900 bg-white px-3 py-1 text-[11px] font-medium text-neutral-900 disabled:opacity-50"
          aria-label="추천"
        >
          <ThumbIcon />
          <span>{upvotes}</span>
