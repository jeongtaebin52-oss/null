...
import { parseOS } from "../../../../../../lib/parse-ua";

type Params = { pageId: string };

/**
 * §31.6 OS/브라우저: enter 이벤트 payload.ua 기준 OS별 방문.
 * GET ?period=7d|30d → { by_os: { name: string, visits: number }[] }
 */
export async function GET(req: Request, context: { params: Promise<Params> }) {
  ...
  const countByOS: Record<string, number> = {};
  for (const r of rows) {
    const name = parseOS(r.ua ?? "");
    countByOS[name] = (countByOS[name] ?? 0) + 1;
  }
  const by_os = Object.entries(countByOS)
    .map(([name, visits]) => ({ name, visits }))
    .sort((a, b) => b.visits - a.visits);

  return NextResponse.json({ period, by_os });
}
